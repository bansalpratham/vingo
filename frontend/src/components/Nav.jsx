    ...
    <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible'>
      ...
      {showSearch && userData.role == "user" && <div className='w-[90%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden' >
        ...
        <div className='w-[80%] flex items-center gap-[10px]'>
          ...
          <input className='px-[10px] text-gray-700 outline-0 w-full' type="text" placeholder='search delicious food...' onChange={(e)=>setQuery(e.target.value)} value={query} />
        </div>
      </div>}
      ...
      <div className='flex items-center gap-[4px]'>
        {userData.role == "user" && (showSearch ? <ImCross size={25} className='text-[#ff4d2d] md:hidden' onClick={() => setShowSearch(false)} /> : <IoIosSearch size={25} className='text-[#ff4d2d] md:hidden' onClick={() => setShowSearch(true)} />)}
        {userData.role == "owner" ? <>
        {myShopData && <>
          ...
          <button className=' flex md:hidden items-center p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]' onClick={()=>navigate("/add-item")} >
            <FaPlus size={20} />
          </button>
        </>}
